...
glm::mat4 KeyframeCollection::interpolateTransformation( float time ) const
{
    ...
    if( !m_keyframes.empty() )
    {
        ...
        float time_laps = result[1].first - result[0].first;
        float ti = time - result[0].first;
        float factor = ti/time_laps;
        ...
        glm::quat orientation1 = result[0].second.getOrientation();
        glm::quat orientation2 = result[1].second.getOrientation();
        glm::quat orientation = glm::slerp(orientation1, orientation2, factor);


        glm::vec3 translation1 = result[0].second.getTranslation();
        glm::vec3 translation2 = result[1].second.getTranslation();
        glm::vec3 translation = glm::lerp(translation1, translation2, factor);


        glm::vec3 scale1 = result[0].second.getScale();
        glm::vec3 scale2 = result[1].second.getScale();
        glm::vec3 scale = glm::lerp(scale1, scale2, factor);
